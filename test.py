...
def eval_psnr(
    ...
    for batch in pbar:
        for k, v in batch.items():
            if k != 'file_name':  # 新增：不要将文件名移到 GPU
                batch[k] = v.cuda()

        inp = batch['inp']
        file_names = batch['file_name']  # 新增：获取文件名
        output_masks = model.infer(inp)
        output_masks_softmax = torch.softmax(output_masks, dim=1)  # 改
    ...
    normed_confusionMatrix = (
        metric_seg.confusionMatrix / metric_seg.confusionMatrix.sum(axis=0)
    )
    ...
    plt.savefig('./save/' + work_dir + '/confusionmatrix.png')
...
if __name__ == '__main__':
    ...
    loader = DataLoader(
        dataset, batch_size=spec['batch_size'], num_workers=4, shuffle=False
    )
    ...
    with torch.no_grad():
        metric1, metric2, metric3, metric4, seg_eval_table = eval_psnr(
            loader,
            model,
