...
class MaskDecoder(nn.Module):
    ...
    def forward(
        ...
        # Prepare output
        masks = masks.unsqueeze(1)  # (1,1,num_cls,256,256)
    ...
    def predict_masks(
        ...
        hs, src = self.transformer(
            src, pos_src, tokens
        )  # hs (1,num_cls+1,256) src (1,4096,256)
        iou_token_out = hs[:, 0, :]  # (1,256)
        mask_tokens_out = hs[:, 1 : (1 + self.num_mask_tokens), :]  # (1,num_cls,256)
        ...
        hyper_in_list: List[torch.Tensor] = []  # num_cls * (1,32)
        for i in range(self.num_mask_tokens):
            hyper_in_list.append(
                self.output_hypernetworks_mlps[i](mask_tokens_out[:, i, :])
            )
        hyper_in = torch.stack(hyper_in_list, dim=1)  # (1, num_cls, 32)
