...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        if multi_scale:
            ...
            # 添加多尺度注意力融合模块
            self.attention_fusion = MultiScaleAttentionFusion(embed_dim, num_heads=8)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.multi_scale:
            ...
            for scale_factor, scale_embed in zip(self.scale_factors, self.scale_embeddings):
                ...
                scale_feat = scale_embed(scaled_x)
                
                if scale_factor != 1.0:
                    scale_feat = F.interpolate(
                        scale_feat.permute(0,3,1,2), 
                        size=(x.shape[2]//self.patch_size, x.shape[3]//self.patch_size),
                        mode='bilinear',
                        align_corners=True
                    ).permute(0,2,3,1)
            ...
            # 使用注意力机制融合多尺度特征
            x = self.attention_fusion(multi_scale_features)
        else:
...
class MultiScaleAttentionFusion(nn.Module):
    ...
    def forward(self, features):
        ...
        for feat in features:
            # [B, H, W, C] -> [B, H*W, C]
            feat_seq = feat.reshape(B, -1, feat.shape[-1])
            features_seq.append(feat_seq)
        
        # 拼接所有尺度的特征
        features_cat = torch.cat(features_seq, dim=1)  # [B, sum(H*W), C]
        
        # 计算注意力
        q = self.q_proj(features_cat).reshape(
            B, -1, self.num_heads, features_cat.shape[-1] // self.num_heads
        ).permute(0, 2, 1, 3)  # [B, num_heads, sum(H*W), head_dim]
        
        k = self.k_proj(features_cat).reshape(
            B, -1, self.num_heads, features_cat.shape[-1] // self.num_heads
        ).permute(0, 2, 1, 3)
        
        v = self.v_proj(features_cat).reshape(
            B, -1, self.num_heads, features_cat.shape[-1] // self.num_heads
        ).permute(0, 2, 1, 3)
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, -1, features_cat.shape[-1])
        x = self.proj(x)
        ...
        for feat in features:
            H, W = feat.shape[1:3]
            feat_len = H * W
            feat_refined = x[:, start_idx:start_idx+feat_len].reshape(B, H, W, -1)
            refined_features.append(feat_refined)
            start_idx += feat_len
        ...
        # 3. 加权融合
        fused_feat = sum(fused_features) / len(fused_features)
        return fused_feat
